...
public ref struct SpanBuilder
{
    ...
    {
        _provider = null;
        _chars = _arrayToReturnToPool = ArrayPool<char>.Shared.Rent(GetDefaultLength(capacity));
        _pos = 0;
    ...
    /// <remarks>This is intended to be called only by compiler-generated code. Arguments are not validated as they'd otherwise be for members intended to be used directly.</remarks>
    public SpanBuilder(int capacity, IFormatProvider? provider)
    {
        _provider = provider;
        _chars = _arrayToReturnToPool = ArrayPool<char>.Shared.Rent(GetDefaultLength(capacity));
        _pos = 0;
    ...
    /// <remarks>This is intended to be called only by compiler-generated code. Arguments are not validated as they'd otherwise be for members intended to be used directly.</remarks>
    public SpanBuilder(int capacity, IFormatProvider? provider, Span<char> initialBuffer)
    {
        _provider = provider;
        _chars = initialBuffer.IsEmpty
            ? _arrayToReturnToPool = ArrayPool<char>.Shared.Rent(GetDefaultLength(capacity))
            : initialBuffer;
        _pos = 0;
        _hasCustomFormatter = provider is not null && HasCustomFormatter(provider);
    }
    ...
    internal static int GetDefaultLength(int capacity) =>
        Math.Max(MinimumArrayPoolLength, capacity);
    ...
    public readonly ReadOnlySpan<char> Text => _chars[.._pos];
    ...
    private void AppendCustomFormatter<T>(T value, string? format)
    {
        ...
        Debug.AssertNotNull(_provider);

        ICustomFormatter? formatter = (ICustomFormatter?)_provider!.GetFormat(typeof(ICustomFormatter))
            ?? throw new InvalidOperationException("An incorrectly written provider said it implemented ICustomFormatter, and then didn't");
    ...
    {
        Debug.AssertInRange(startingPos, 0,_pos);
        Debug.AssertNotZero(alignment);
    ...
    private void Grow(int additionalChars)
    {
        ...
        Debug.AssertGreaterThan(additionalChars, _chars.Length - _pos);
    ...
    private void GrowCore(uint requiredMinCapacity)
    {
        ...
        int arraySize = (int)Math.Max(newCapacity, MinimumArrayPoolLength);
    ...
    public void Dispose()
    {
        if (_arrayToReturnToPool is { } array)
        {
