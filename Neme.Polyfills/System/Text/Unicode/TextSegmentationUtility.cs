...
internal static class TextSegmentationUtility
{
    ...
    private static readonly DecodeFirstRune<Rune> _runeDecoder =
        static OperationStatus (ReadOnlySpan<Rune> input, out Rune rune, out int runesConsumed) =>
        {
            if (input.Length > 0)
            {
                rune = input[0];
    ...
    public static int GetLengthOfFirstExtendedGraphemeCluster(ReadOnlySpan<Rune> input)
    {
    ...
    private ref struct Processor<T>
    {
        ...
        public void MoveNext()
        {
            ...
            CurrentType = CharUnicodeInfoPolyfill.GetGraphemeClusterBreakType(thisRune);
        }
    }
}
