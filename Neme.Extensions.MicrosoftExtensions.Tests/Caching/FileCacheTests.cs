...
public sealed class FileCacheTests : IDisposable
{
    ...
    {
        _testCacheDirectory = Path.Combine(Path.GetTempPath(), $"FileCache_Tests_{Guid.NewGuid()}");
    ...
    private FileCache CreateFileCache(
        ...
        FileOptions? defaultFileOptions = null)
    {
        var options = Options.Create(new FileCacheOptions
        {
            ...
            DefaultFileOptions = defaultFileOptions ?? (FileOptions.Asynchronous | FileOptions.SequentialScan)
        });
