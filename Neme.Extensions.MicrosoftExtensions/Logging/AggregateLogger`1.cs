...
public sealed class AggregateLogger<T> : ILogger<T>
{
    private readonly ImmutableArray<ILogger<T>> _loggers;

    public AggregateLogger(ImmutableArray<ILogger<T>> loggers)
    {
        _loggers = loggers;
    }

    public AggregateLogger(IEnumerable<ILogger<T>> loggers)
    {
        _loggers = loggers.ToImmutableArray();
    }

    public AggregateLogger(params ReadOnlySpan<ILogger<T>> loggers)
    {
        _loggers = loggers.ToImmutableArray();
    }
